...
//检测QQ快捷登录开启状态
    ...
    sysmsg('<h2>系统未开启QQ快捷登录功能</h2><ul><li><font size="4">'.$date.'</font></li>',true);
}
...
if($row && $row['active']==0){
	...
	exit("<script language='javascript'>alert('QQ快捷登录成功');window.location.href='./';</script>");
	    ...
	    if($conf['oauth_qq_type']=='qq'){
	        $jump = './login_qqsign.php?token='.$text;
	    }else{
	        $jump = './';
	    }
	exit("<script language='javascript'>alert('当前QQ未绑定账户');window.location.href='./';</script>");
	}
}
?>
