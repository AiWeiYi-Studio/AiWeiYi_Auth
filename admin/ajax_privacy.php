...
case 'del':
    ...
    $sql="DELETE FROM website_privacy where id = '{$id}'";
    if(!$id){
        $result = array(
            "code"=>-1,
            "msg"=>"必要参数不能为空"
...
case 'add':
    ...
    $log_sql = "insert into `website_log` (`uid`,`ip`,`city`,`type`,`content`,`date`,`user`) values ('".$udata['uid']."','".$clientip."','".$city."','发布','发布悄悄话：".$text."','".$date."','admin')";
    if(!$text){
        $result = array(
            "code"=>-1,
            "msg"=>"内容为空"
