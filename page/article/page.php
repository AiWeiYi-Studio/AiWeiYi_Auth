...
$DB->query("update website_article set number='{$row['number']}'+1 where id='{$row['id']}'");
...
if($conf['rewrite_article']=='1'){
    $head = '
        ...
        <a href="/article/'.$row['id'].'.html">'.$row['title'].'</a>
        ';
    $baidu_post_url = $siteurls.'article/'.$id.'.html';
}else{
    $head = '
        <a href="/">网站首页</a> \ 
        <a href="/page/article/index.php">文章列表</a> \ 
        <a href="/page/article/page.php?id='.$row['id'].'">'.$row['title'].'</a>
        ';
    $baidu_post_url = $siteurls.'page/article/page.php?id='.$id.'';
}
$baidu_post = array(
    ''.$baidu_post_url.''
);
