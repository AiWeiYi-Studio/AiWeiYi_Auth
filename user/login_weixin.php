...
//检测微信快捷登录开启状态
}elseif($conf['oauth_api_wechat']=='0'){
    sysmsg('<h2>系统未开启微信快捷登录功能</h2><ul><li><font size="4">'.$date.'</font></li>',true);
}
...
// 反之则执行登录
    ...
    if($row && $row['active']==0){
        ...
        exit("<script language='javascript'>alert('微信快捷登录成功');window.location.href='./';</script>");
	}else{
	    exit("<script language='javascript'>alert('当前微信未绑定账户');window.location.href='./login_sign.php?text=".$text."&type=weixin';</script>");
	}
}
?>
